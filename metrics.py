...
def parse_args():
    ...
    evidence.add_argument('--evidence-post-pipeline', required=False, metavar='<path>', type=str, help=(
        'Directory with the Parquet evidence files generated by the ETL pipeline for the appropriate dataset. Can be '
        'found in gs://open-targets-data-releases/{RELEASE}/output/etl-parquet/evidence.'))

    # All of the remaining datasets.
    parser.add_argument(
        '--evidence-failed', required=False, metavar='<path>', type=str, help=(
            'Failed evidence files, gs://open-targets-data-releases/{RELEASE}/output/etl-parquet/evidenceFailed.'))
    parser.add_argument(
        '--associations-direct', required=False, metavar='<path>', type=str, help=(
            'Direct association files, ...'))  # FIXME: TBC.
    parser.add_argument(
        '--associations-indirect', required=False, metavar='<path>', type=str, help=(
            'Indirect association files, ...'))  # FIXME: TBC.
    parser.add_argument(
        '--diseases', required=False, metavar='<path>', type=str, help=(
            'Disease information, gs://open-targets-data-releases/{RELEASE}/output/etl-parquet/diseases.'))
    parser.add_argument(
        '--drugs', required=False, metavar='<path>', type=str, help=(
            'ChEMBL dataset, gs://open-targets-data-releases/{RELEASE}/input/annotation-files.'))
