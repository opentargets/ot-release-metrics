...
def parse_args():
    ...
    pre_pipeline_parser.add_argument(
        '--evidence', required=True, metavar='<path>', type=str, help=(
    ...
    post_pipeline_parser.add_argument(
        '--drugs', required=False, metavar='<path>', type=str, help=(
            'ChEMBL dataset, found in ../../../input/annotation-files'))
    ...
    parser.add_argument(
        '--logFile', required=False, type=str, help=(
...
def main(args):
    ...
    if args.run_type == 'pre-pipeline':
        evidence = spark.read.option('recursiveFileLookup', 'true').json(args.evidence)
    elif args.run_type == 'post-pipeline':
        evidence = read_file_if_provided(spark, args.evidence)
